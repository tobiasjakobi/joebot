	...
	*Chat.szChatText = 0;
	...
	if(g_iMapType == MT_AS){
		if(!TEq(fLastVIPScan,gpGlobals->time,2)){
			...
			//std::cout << "searching for vip" << std::endl;
			g_pVIP = 0;
...
void CBotCS :: Think(void){
	//std::cout << pEdict->v.view_ofs.Length()<<std::endl;
	//std::cout << STRING(pEdict->v.netname) << std::endl;
	edict_t *pent=0;
	...
	if ((pEdict->v.health < 1) || (pEdict->v.deadflag != DEAD_NO))
	{
		...
		//std::cout << "deadthinkend" << std::endl;
		return;
	}
#ifdef _DEBUG
